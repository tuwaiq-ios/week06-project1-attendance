...
class StudentsScreen: UIViewController {
    ...
    private func setupAddStudentButton() {
        ...
        addStudentButton.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -200).isActive = true
        addStudentButton.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -40).isActive = true
    ...
    private func setupStackView() {
        ...
        attendanceScreenButton.setTitleColor(.white, for: .normal)
        attendanceScreenButton.addTarget(self, action: #selector(daysButtonTapped), for: .touchUpInside)
        
        studentsButton.setTitle("Students", for: .normal)
        studentsButton.setTitleColor(.white, for: .normal)
        
        let stackView = UIStackView(arrangedSubviews: [studentsButton, attendanceScreenButton])
        stackView.backgroundColor = UIColor(red: (147/255), green: (163/255), blue: (148/255), alpha: 1).withAlphaComponent(0.9)
        stackView.layer.cornerRadius = 20
        stackView.clipsToBounds = true
        stackView.distribution = .fillEqually
        stackView.translatesAutoresizingMaskIntoConstraints = false
        view.addSubview(stackView)
        stackView.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -110).isActive = true
